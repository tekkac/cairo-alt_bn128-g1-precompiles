from core.bigint import BigInt3, UnreducedBigInt3, nondet_bigint3, UnreducedBigInt5, bigint_mul
from core.field import is_zero, verify_zero5, fq_diff
...
func eq_g1{range_check_ptr}(a: G1Point, b: G1Point) -> (res: felt) {
    alloc_locals;
    let (local x_diff) = fq_diff(a.x, b.x);
    let (local is_x_0) = is_zero(x_diff);
    let (local y_diff) = fq_diff(a.y, b.y);
    let (local is_y_0) = is_zero(x_diff);
    let res = is_x_0 * is_y_0;
    return (res=res);
}
